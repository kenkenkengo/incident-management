...
import { create, close, findActiveByChannel } from
...
export const handleIncidentCommand = async ({
  ...
  if (subcommand === "start") {
    ...
    const incident = await create({ title }, command.channel_id, command.user_id)
    await respond(`🚨 インシデント開始: "${incident.title}"\nこのチャンネルのメッセージを記録しています。`);
    return;
  }
  ...
  if (subcommand === "end") {
    ...
    await respond(`✅ インシデント終了: "${active.title}"\nメッセージの記録を停止しました。`);
    return;
  }
