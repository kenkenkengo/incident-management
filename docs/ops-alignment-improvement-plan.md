...
- **#27 `/incident end` でトラブル報告をSlackへ自動投稿**: 終了時に定型フォーマット（＜発生日時＞＜時系列＞＜発生事象＞＜原因＞＜影響＞＜対応＞＜今後の対策＞）のトラブル報告を Bedrock で生成し専用chへ投稿（時刻JST・時系列はチャットログから要約）。冗長な終了サマリーは廃し**報告本文＋PMリンクのみ**（生成失敗時は最小メッセージにフォールバック）。ワーカーに `bedrock:InvokeModel` 権限付与・timeout 120秒。
